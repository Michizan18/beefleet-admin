...
const Clientes = () => {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      setClients([]); // Limpiar clientes en caso de error
    } finally {
      setLoading(false);
    }
  };
  ...
  try {
    ...
    return result.client; // Retornar solo el cliente creado
  } catch (error) {
...
const deleteClient = async (clientId) => {
  ...
  const filteredClients = clients.filter((client) => {
    // Filtrar por término de búsqueda con validación de campos
    const matchesSearch = 
      (client.documento && client.documento.toString().includes(searchTerm)) ||
      (client.nombre_cliente && client.nombre_cliente.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (client.apellido_cliente && client.apellido_cliente.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (client.empresa && client.empresa.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (client.ciudad && client.ciudad.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleSubmitNewClient = async (e) => {
    ...
    try {
      setLoading(true);
      const newClientData = await createNewClient(newClient);
      ...
      alert(`Cliente ${newClientData.nombre_cliente} ${newClientData.apellido_cliente} creado exitosamente`);
    } catch (error) {
 ...
 const handleDeleteClient = async (clientId) => {
  // Encontrar el cliente para mostrar su nombre en la confirmación
  const clientToDelete = clients.find(client => client.id_cliente === clientId);
  const clientName = clientToDelete ? 
    `${clientToDelete.nombre_cliente} ${clientToDelete.apellido_cliente}` : 
    'este cliente';

  if (window.confirm(`¿Está seguro de que desea eliminar a ${clientName}?`)) {
    try {
      setLoading(true);
                ...
                <tbody>
                  {filteredClients.map((client) => (
                    <tr key={client.id_cliente}>
                      <td>
                        <div>
                          <small className="text-muted">{client.tipo_documento || 'N/A'}</small>
                          <br />
                          <strong>{client.documento || 'N/A'}</strong>
                        </div>
                      </td>
                      <td>
                        <div className="d-flex align-items-center">
                          <FaUserCircle className="me-2 text-warning" />
                          {`${client.nombre_cliente || ''} ${client.apellido_cliente || ''}`.trim() || 'N/A'}
                        </div>
                      </td>
                      <td>
                        <div className="d-flex align-items-center">
