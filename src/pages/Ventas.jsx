...
import { Card, Table, Button, InputGroup, Form, Modal, Badge, Row, Col } from 'react-bootstrap';
import { FaUser, FaCheckCircle, FaSearch, FaCalendarAlt, FaFileInvoiceDollar, FaEdit, FaTrashAlt, FaPlus, FaSave } from 'react-icons/fa';
...
const Ventas = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [salesResponse, loadsResponse] = await Promise.all([
        fetch('http://localhost:3001/api/sales/', { method: 'GET', headers }),
        fetch('http://localhost:3001/api/loads/', { method: 'GET', headers }),
      ]);

      if (!salesResponse.ok) {
        throw new Error(`Error al cargar las ventas: ${salesResponse.statusText}`);
      }
      ...
      if (!loadsResponse.ok) {
        throw new Error(`Error al cargar las cargas: ${loadsResponse.statusText}`);
      }
  ...
  const deleteSale = useCallback(async (saleId) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorText || 'Error al eliminar la venta');
      }
  ...
  return (
    <LayoutBarButton userData={userData}>
      ...
      {/* Modal de éxito para eliminar venta */}
      <Modal
